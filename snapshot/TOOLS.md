...
**analyze_image provider failures: fall back to native vision, don't retry.** Dominant mode (15/27 lifetime failures): "All provider vision candidates failed: No text response from vision model" — provider-side flakiness, not path misuse. On this error, do not re-call analyze_image; read the image directly with `read_file` (native vision). Second mode (2×): 413 "request body exceeds configured limit" — preflight file size and downscale (`sips -Z 2048`) before calling. After ONE failure, switch method; a 5-failure streak on 2026-08-21 16:50–16:56 shows the fallback wasn't applied. Violations: 1.
...
- **`edit_file` parallel same-file race (2026-08-23; REPRODUCED & CONFIRMED as "the phantom" 2026-09-07):** never batch multiple `edit_file` calls to the SAME file in one tool block. They resolve against the same original snapshot; the last write wins and silently wipes the others. Same file → strictly sequential edits, re-read between when in doubt. **2026-09-07 controlled reproduction** (phantom-scratch.clj, 4 parallel edits): 4 success receipts, only 2 hunks on disk, tool warnings "another agent was writing" on exactly the 2 lost hunks; FS watcher saw only the 2 winning writes. **Attribution correction:** the "phantom writer" blamed across 2026-09-06/07 sessions (signature-eats, vanished hunks, write-gate staleness refusals) was substantially THIS known race violating this rule — the rule predates the incidents. Residual unattributed anomalies remain (kondo binary deletion from .tools/bin; foreign `./~/Desktop/tinydb` tree from an unexpanded tilde in some session's script; `flow_debug.clj` from the flow session) — real foreign artifacts exist, but lost-update attribution now defaults to self-race, not an external agent.
...
- **adb on this Mac:** `exec-out screencap -p` corrupts PNGs (CRLF) — `shell screencap -p /sdcard/x.png` + `pull` instead. uiautomator dump fails with "could not get idle state" while the player screen runs (10Hz polling); use screenshots. Fold 7 SM-F966U1 USB link is intermittent — long adb ops should expect mid-command drops.
