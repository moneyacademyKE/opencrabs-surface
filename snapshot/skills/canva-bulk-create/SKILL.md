...
> **DEGRADED (verified 2026-09-10):** the live Canva MCP surface (33 tools) does NOT expose `autofill-design` — this skill's core verb. Steps 3-5 will fail until Canva ships it (or the account gains Enterprise autofill scope). Steps 1-2 (data parsing, template discovery via `search-brand-templates` + `get-brand-template-dataset`) work; fall back to `create-design-from-brand-template` per row and edit via the transaction protocol.
