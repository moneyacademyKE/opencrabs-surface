...
4. **Python only when justified**: the project is Python, a required library/tool is Python-only, or `bb`/shell/project-native tooling would make the task materially worse. Do not reach for Python out of habit.
...
Prefer files/modules to stay **under 250 LOC**. Smaller files are easier to scan, safer to refactor, and better for rapid iteration while code is still in progress.
...
4. Log the probe result to `state/inflight.md`.

This prevents silent amnesia. One extra mental pass, near-zero cost, catches the worst compaction failure mode.
