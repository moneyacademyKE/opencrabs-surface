...
2. **Whole-file rewrite > paren surgery**: if a file is fully read, rewriting it whole beats line edits. For unavoidable region splices: rebuild from the original's exact bytes at machine-located anchors and reader-verify forms as data — hand-counted closers cost 8 probes (2026-09-06 H2). **Parse check proves balance, not sanity**: an off-by-one-paren splice shredded theseus config.clj yet parsed clean (`c528cea`) — after structural edits also assert expected defn names/count before tests.
