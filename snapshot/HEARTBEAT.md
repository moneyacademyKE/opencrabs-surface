...
0. **Config integrity probe** (cheap, do first): `grep -Fn 'chat_permissions = "{' ~/.opencrabs/config.toml` — **`-F` is mandatory.** `grep` is aliased to `rg` on this host, so the bare `{` parses as a regex quantifier and the search dies with `rg: regex parse error`, exit **2**. Read the exit code, not the silence: **1** = no match = healthy, **0** = corruption found, **2** = the probe itself broke and proved nothing. (Caught 2026-09-14 after eight consecutive pulses reported "exit 1 → clean" off an exit-2 error.) A string where a table belongs was the 2026-09-07 corruption (`"{}"` from an unknown writer ~14:44 local) that killed the userbot silently (B7). If the probe trips or the config looks wrong, run `opencrabs doctor`, restore from `~/.opencrabs/backups/`, and notify.
